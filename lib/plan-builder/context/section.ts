 ...
 */
import type { ContextField, PlanBusinessContext } from "./build";
...
export const SECTION_CONTEXT_RULES: Record<string, Rule> = {
  "overview/summary": { fields: [...CORE, "identity.region", "identity.stage", "marketing.acquisitionModel", "revenue.unitPrice", "revenue.volume", "classification.modelTags"], hints: true, finance: true },
  ...
  "market/products": { fields: ["solution.mainOffer", "solution.description", "solution.differentiator", "revenue.unitPrice", "revenue.model", "classification.modelTags"], hints: true },
  "market/segments": { fields: ["customer.target", "customer.persona", "identity.region", "operations.coverage", "classification.operationTags"] },
  "market/personas": { fields: ["customer.target", "customer.persona", "customer.budget", "customer.channels", "marketing.acquisitionModel", "problem.statement"] },
  ...
  "strategy/distribution": { fields: ["operations.delivery", "operations.coverage", "operations.venueType", "operations.capacity", "revenue.model", "classification.operationTags"], hints: true },
  "strategy/price": { fields: ["revenue.unitPrice", "revenue.model", "customer.budget", "solution.mainOffer", "classification.modelTags"], hints: true, finance: true },
  "strategy/promotion": { fields: ["customer.target", "marketing.channels", "marketing.acquisitionModel", "marketing.message", "marketing.budget", "customer.channels", "classification.operationTags"], hints: true },
  "strategy/people": { fields: ["team.size", "operations.who", "operations.capacity", "team.ownerExperience", "classification.operationTags"], hints: true },
  ...
  "financials/revenue": { fields: ["revenue.model", "revenue.streams", "revenue.unitPrice", "revenue.volume", "operations.capacity", "classification.modelTags"], hints: true, finance: true },
  "financials/staffing": { fields: ["operations.who", "team.size", "operations.capacity"], finance: true },
  "financials/expenses": { fields: ["operations.venueType", "operations.delivery", "classification.modelTags", "marketing.budget"], hints: true, finance: true },
  ...
  "summary/executive": { fields: [...CORE, "identity.region", "solution.differentiator", "marketing.acquisitionModel", "revenue.unitPrice", "revenue.volume", "team.ownerExperience", "funding.needs", "goals.main"], hints: true, finance: true },
};
...
export function contextForSection(sectionKey: string, ctx: PlanBusinessContext | null | undefined): SectionBusinessContext | undefined {
  ...
  const out: SectionBusinessContext = { confirmed, inferred, hints, unknowns };
  if (rule.finance && ctx.finance.summary) out.finance = ctx.finance.summary;
  if (!confirmed.length && !inferred.length && !hints.length && !out.finance) return undefined;
  return out;
}
