    ...
    <section className="operations-panel">
      ...
      {tab === "procurement" && (
        <div className="operations-body">
          ...
          <div className="quote-list">{workspace.supplierQuotes.map((entry) => <article key={entry.id}><span className={entry.status}>{entry.status === "verified" ? "원문 확인" : "확인 중"}</span><div><strong>{entry.supplierName} · {entry.itemName}</strong><p>{won(entry.unitPrice)} × 최소 {entry.minimumOrderQuantity}개 · 납기 {entry.leadTimeDays}일</p></div>{entry.sourceUrl && <a href={entry.sourceUrl} target="_blank" rel="noreferrer"><ExternalLink /></a>}<button aria-label="견적 삭제" onClick={() => setWorkspace({ ...workspace, supplierQuotes: workspace.supplierQuotes.filter((quoteItem) => quoteItem.id !== entry.id) })}>삭제</button></article>)}{!workspace.supplierQuotes.length && <p className="operation-empty">실제 공급처 견적이 아직 없습니다.</p>}</div>
