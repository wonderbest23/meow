  ...
  return <section className={styles.sourceUpdate} aria-label="계획서 갱신 검토">
    ...
    {open && <div className={styles.updateBody}>
      ...
      {!pending && <>
        ...
        <div className={styles.updateActions}><Link href={`/plan/document?planId=${encodeURIComponent(planId)}`}>문서 직접 확인</Link><button disabled={!keys.length || loading || !canUpdate} onClick={() => void inspect()}>선택한 전송 자료 확인</button></div>
