...
import { ArrowUp, ChevronRight, Paperclip, FileCheck2, Check, Lightbulb, FileText, Store } from "lucide-react";
...
const ENTRY_ICONS = [Lightbulb, FileText, Store];
...
function BusinessCoach() {
  ...
  const [entryMode, setEntryMode] = useState<"loading" | "new" | "resume">("loading");
  ...
  useEffect(() => {
    ...
    const mode = chatEntryIntent(query);
    setEntryMode(mode);
    const clearPrompt = () => {
      if (!query.has("prompt")) return;
      query.delete("prompt");
      if (mode === "new") query.set("new", "1");
      window.history.replaceState(null, "", `/plan/chat${query.size ? `?${query}` : ""}`);
    };
    // Updating the URL after a successful send must not reset the live conversation.
    if (id && id === planRef.current?.planId) {
    ...
    setLoaded(false); setLoadFailed(false); setError(""); setLogin(false);
    setBusy(false); setOptimistic(null); setAttachment(null); setView("chat");
    setChanged([]); setUnseen(false); setEditDirty(false); follow.current = true;
    accept({ plan: null, paid: false, runStatus: null });
    ...
    if (mode === "new") { setLoaded(true); clearPrompt(); return () => controller.abort(); }
    refresh(id ?? undefined, controller.signal)
      .then(snapshot => {
        if (controller.signal.aborted) return;
        setLoaded(true);
        if (!id && snapshot) {
          storageKey.current = `coach-input:${snapshot.planId}`;
          try { setText(sessionStorage.getItem(storageKey.current) || ""); } catch { setText(""); }
          window.history.replaceState(null, "", businessChatHref(snapshot.planId));
        } else clearPrompt();
      })
      .catch(e => { if (!controller.signal.aborted) { setError(e.message); setLoadFailed(true); } })
      .finally(() => { if (!controller.signal.aborted) setLoaded(true); });
    return () => controller.abort();
  }, [queryString, refresh, accept]);
  useEffect(() => {
    if (!loaded) return;
  ...
  useEffect(() => {
    ...
    if (follow.current) {
      conversationRef.current?.scrollTo({ top: conversationRef.current.scrollHeight, behavior: "instant" });
      setUnseen(false);
    } else if (plan?.coach.messages.length || optimistic || busy) setUnseen(true);
  ...
  useEffect(() => {
    ...
    let inFlight = false;
    const controller = new AbortController();
    const poll = async () => { if (inFlight) return; inFlight = true; try { await refresh(plan.planId, controller.signal); } catch { if (!controller.signal.aborted) setConnectionError(true); } finally { inFlight = false; } };
  ...
  async function submit(message = text, action: "message" | "prepare" = "message", retry = false) {
    if (!loaded || (!plan && entryMode !== "new") || submitting.current || blocked) return;
    const epoch = routeEpoch.current;
      ...
      <section id="business-chat-pane" className={`${styles.chatPane} ${view !== "chat" ? styles.mobileHidden : ""}`} aria-label="사업 기획 대화">
        <div ref={conversationRef} className={styles.conversation} onScroll={e => { const el = e.currentTarget; follow.current = el.scrollHeight - el.scrollTop - el.clientHeight < 100; if (follow.current) setUnseen(false); }}>
          <div className={styles.thread}>
            <article className={`${styles.assistant} ${!started ? styles.firstMessage : ""}`} aria-label="오늘창업의 첫 메시지">{!started ? <CoachWelcome /> : <><CoachSpeaker /><p>어떤 사업을 생각하고 계세요?</p><p className={styles.greetingNote}>막연한 생각부터 편하게 이야기해 주세요.</p></>}</article>
            {!started && <div className={styles.entryOptions} aria-label="대화 시작 선택지">{ENTRY_OPTIONS.map((option, index) => { const Icon = ENTRY_ICONS[index]; return <button type="button" key={option} style={{ animationDelay: `${index * 80 + 150}ms` }} disabled={!loaded || blocked} onClick={() => { pendingId.current = null; void submit(option); }}><i className={styles.choiceIcon} aria-hidden="true"><Icon size={28} strokeWidth={1.8} /></i><span>{option}</span><ChevronRight size={17} aria-hidden="true" /></button>; })}</div>}
            {plan?.coach.messages.map((message, index) => <CoachMessage key={message.id} role={message.role} className={message.role === "user" ? styles.user : styles.assistant}>
            ...
            {((plan?.job && !plan.coach.messages.some(message => message.id === plan.job?.message.id)) || optimistic) && <CoachMessage role="user" className={styles.user}><p>{plan?.job && !plan.coach.messages.some(message => message.id === plan.job?.message.id) ? plan.job.message.text : optimistic}</p><small>{working ? "접수됨" : busy ? "보내는 중" : "답변 대기"}</small></CoachMessage>}
            {(busy || working) && <div className={styles.thinking} role="status"><CoachSpeaker /><div className={styles.thinkingLine}><p>{working ? PHASES[plan!.job!.phase] : "요청을 보내고 있어요"}</p><div className={chatUi.typingDots} aria-hidden="true"><i /><i /><i /></div></div><small>{working && plan?.job?.durable ? "화면을 닫아도 서버에서 계속 만들어요." : "완료될 때까지 이 화면을 유지해 주세요."}</small></div>}
            ...
            {hasBrief && !blocked && <CoachResultCard className={styles.readyNotice} label={documentCurrent ? "계획서 완성" : needsDocumentUpdate ? "수정한 사업안" : "함께 정리한 사업안"} title={plan?.title ?? "내 사업안"} description={needsDocumentUpdate ? "바꾼 내용을 계획서에도 반영할 수 있어요." : "고객 · 상품 · 비용 · 시작 방법"} onOpen={() => { setView("brief"); if (window.matchMedia("(min-width:901px)").matches) document.querySelector<HTMLElement>('[aria-label="내 사업안 결과"] h1')?.focus(); }} />}
        ...
        <footer className={styles.composer}>
          ...
          <form className={chatUi.composerShell} aria-busy={busy || working} onClick={e => { if (e.target === e.currentTarget) input.current?.focus(); }} onSubmit={e => { e.preventDefault(); void submit(); }}>
            ...
            <textarea ref={input} aria-label="사업에 대해 말씀해주세요" placeholder={started ? "메시지를 입력하세요" : "생각을 들려주세요"} value={text} maxLength={2500} rows={1} disabled={blocked} onChange={e => { setText(e.target.value); pendingId.current = null; }} onKeyDown={e => { if (e.key === "Enter" && !e.shiftKey && !e.nativeEvent.isComposing && !window.matchMedia("(max-width: 900px)").matches) { e.preventDefault(); void submit(); } }} />
            <button type="submit" className={`${styles.send} ${chatUi.sendButton} ${text.trim() || attachment ? styles.sendReady : ""}`} aria-label="보내기" title="보내기" disabled={!loaded || blocked || (!text.trim() && !attachment)}><ArrowUp size={22} /></button>
          </form>
        </footer>
      </section>
