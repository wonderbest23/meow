...
function IntakeWorkspace({ onPrepared, onDesignComplete }: BusinessIntakeProps) {
  ...
  const split = useChatSplit(plan?.coreComplete
    ? { key: "oneulstart:intake-split:complete", defaultShare: 42, minChatPx: 360, minSidePx: 340, maxShare: 72 }
    : { key: "oneulstart:intake-split:answering", defaultShare: 64, minChatPx: 360, minSidePx: 340, maxShare: 72 });
  ...
  const designCurrent = !!plan?.coach.design && plan.coach.design.sourceRevision === (plan.coach.documentRevision ?? plan.coach.revision);
        ...
        <div ref={conversation} data-intake-conversation className={styles.conversation}
          ...
          {!loaded ? <p className={styles.loading} role="status"><LoaderCircle size={21} className={styles.spinner} aria-hidden="true" />사업 정보를 불러오는 중입니다.</p> : !plan ? replyTurn ? <><div className={styles.assistantMessage}><ChatSpeaker /><p>어떤 사업을 생각하고 계세요?</p></div><article className={styles.userMessage} data-coach-message="user"><p>{replyTurn.initialText}</p></article><div ref={currentTurn}><ReplyTyping /></div></> : newEntry ? <EntryChoices disabled={blocked} initialMessage={draft.introMessage} onStart={startFromCard} /> : !loadFailed && <section className={styles.empty}><h1>저장한 사업이 없습니다</h1><Link href="/plan/chat?new=1" className={styles.primaryButton}>새 사업 기획<ArrowRight size={18} aria-hidden="true" /></Link><Link href="/plan" className={styles.textLink}>내 사업으로</Link></section> : <>
            ...
            <div ref={currentTurn} className={styles.currentTurn} aria-busy={!!replyTurn}>
              {replyTurn ? <ReplyTyping /> : intentConfirmation || (question ? <QuestionForm key={`${question.id}:${draft.editingId ?? "current"}`} inChat question={question} snapshot={plan} draft={questionDraft} editing={!!draft.editingId} disabled={blocked} onChange={answer => editDraft({ ...draftRef.current, mode: "answer", answers: { ...draftRef.current.answers, [question.id]: { ...answer, label: question.label } } })} onAnswer={answerQuestion} onCancel={() => { follow.current = true; writeDraft({ ...draftRef.current, editingId: null }); }} /> : draft.editingId ? <section className={styles.complete}><h2>이전 질문의 입력이 남아 있어요</h2><p>현재 사업 정보에 맞춰 질문 구성이 달라졌습니다.</p><button type="button" className={styles.secondaryButton} onClick={() => writeDraft({ ...draftRef.current, editingId: null })}>현재 질문으로</button></section> : <section className={styles.complete}><ChatSpeaker /><h2>이야기해 주신 내용을 정리했어요</h2><p>사업 요약을 확인하고 더 필요한 이야기를 이어가세요</p>{aiBusy && <JobProgress snapshot={plan} announce />}{plan.coreComplete && plan.coach.ready && !designCurrent && !aiBusy && <div className={styles.nextStep} data-active><strong className={styles.nextStepLabel}><ArrowRight size={14} aria-hidden="true" />다음 단계</strong><button type="button" className={styles.primaryButton} disabled={blocked} onClick={() => void send({ action: "design" })}><Sparkles size={18} aria-hidden="true" />다음 단계: 사업안 만들기</button><small className={styles.nextStepHint}>답변을 바탕으로 AI가 사업안을 정리해요. 보통 20~30초 걸리고, 화면을 나가도 서버에서 계속 진행돼요.</small></div>}<button type="button" className={styles.mobileSummaryButton} onClick={() => setView("summary")}>입력한 사업 요약<ArrowRight size={17} aria-hidden="true" /></button>{!plan.intake.detailsRequested && plan.coreComplete && <button type="button" className={styles.secondaryButton} disabled={blocked} onClick={() => void send({ action: "details" })}>상세 질문 {structureQuestions(plan.intake.mode, plan.structure?.values).length + detailQuestions(plan.intake.sector).length}개 추가</button>}</section>)}
            </div>
          </>}
