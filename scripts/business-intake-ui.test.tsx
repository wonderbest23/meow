...
async function main() {
  ...
  const { EntryChoices, QuestionForm, BusinessSummary, ConversationHistory, ExtractionReview, SavedNotes, AnswerHistory, ReplyTyping, JobProgress, NextStepAction } = await import("../app/plan/chat/intake-ui/IntakePanels");
  const noop = () => {};
  ...
  for (const text of ["입력한 사업 요약", "소규모 매장의 예약 업무를 돕는 소프트웨어", "AI가 제안한 시작 범위", `상세 질문 ${structureQuestions(summarized.intake.mode, summarized.structure?.values).length + detailQuestions(summarized.intake.sector).length}개 추가`, "다음 단계"]) assert.ok(summary.includes(text), `summary is missing: ${text}`);
  assert.equal((summary.match(/만들기<\/button>/g) ?? []).length + (summary.match(/계획서 열기<\/a>/g) ?? []).length >= 1, true, "the summary offers one next step");
  assert.ok((summary.match(/만들기<\/button>/g) ?? []).length <= 1, "never two make-buttons at once");
  ...
  assert.ok(needsDesign.includes("사업안 만들기</button>") && !needsDesign.includes("계획서 만들기</button>"), "only the design button shows before a design exists");
  assert.ok(needsDesign.indexOf("기본 질문 입력 완료") < needsDesign.indexOf("다음 단계") && needsDesign.indexOf("다음 단계") < needsDesign.indexOf("내 사업 구상"), "the next step sits at the top of the summary");
  const needsPlan = nextMarkup({}, currentDesign);
  assert.ok(needsPlan.includes("계획서 만들기</button>") && !needsPlan.includes("사업안 만들기</button>"), "with a current design only the plan document button shows");
  const opened = nextMarkup({ hasDocuments: true }, currentDesign);
  assert.ok(opened.includes("계획서 열기</a>") && !opened.includes("만들기</button>"), "after the documents exist the step is opening them");
  const hiddenActions = nextMarkup({}, { design: undefined }, false);
  assert.ok(!hiddenActions.includes("다음 단계") && !hiddenActions.includes("상세 질문"), "when the chat shows the next step the summary shows no buttons");
  ...
  const chatStep = renderToStaticMarkup(<NextStepAction snapshot={nextSnapshot({}, { design: undefined })} prepared={false} disabled={false} aiBusy={false} onDesign={noop} onPrepare={noop} secondary={<button type="button">상세 질문 6개 추가</button>} />);
  assert.match(chatStep, /class="nextStepRow">[\s\S]*사업안 만들기<\/button><button type="button">상세 질문 6개 추가<\/button><\/div>/, "the details button shares one row with the next step");
  ...
  assert.ok(busySummary.includes('role="progressbar"') && busySummary.includes("27%") && !busySummary.includes("사업안 만들기</button>"), "while the job runs the gauge takes the button's place");
